...
## 12. Swing Biomechanics Across Clubs (March 23)

Research-backed (Liu et al. 2025): Driver, 5-iron, and 7-iron have distinct kinematic and dynamic characteristics. The driver uses slow energy accumulation, delayed acceleration, and horizontal sweeping — the power chain takes 18-22ms longer than irons. The 5-iron balances power and control through frontal angular impulse coordination. The 7-iron uses compact transitions and torso-driven precision. The proximal-to-distal kinematic sequence is consistent across clubs, but the TIMING of the sequence changes significantly. Coaching drills for one club category (e.g., iron strike) will not transfer to another (e.g., driver) because they train different motor patterns. This is why the AI insight should recommend distinct coaching blocks when recommending a club-category pivot.

---

## 13. Player Journey as Engagement Platform (March 23)

The Player Journey timeline enables derived features from existing data: coach intervention alerts on player gaps/regression, milestone celebrations (career low, goal achieved, best round), player re-engagement nudges, academy-level analytics (most improved, retention by coaching frequency, thread-to-improvement correlations), competitive features (improvement challenges, leaderboards). All are views on the persistent record — no new data capture required.